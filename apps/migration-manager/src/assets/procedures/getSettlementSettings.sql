DROP PROCEDURE IF EXISTS `main`.`getSettlementSettings`;

CREATE PROCEDURE getSettlementSettings(
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    INTO procedureError;
  ELSE
    -- Build the settlement settings JSON
    SELECT JSON_OBJECT(
