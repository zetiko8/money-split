...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    SELECT JSON_OBJECT('invitation', 'RESOURCE_NOT_FOUND')
  ...
  ELSE
    ...
    IF v_invitationAccepted = 1 THEN
      SELECT JSON_OBJECT('invitation', 'INVITATION_ALREADY_ACCEPTED')
      INTO validationErrors;
    -- Check if invitation is already rejected
    ELSEIF v_invitationRejected = 1 THEN
      SELECT JSON_OBJECT('invitation', 'INVITATION_ALREADY_REJECTED')
      INTO validationErrors;
    ELSE
      ...
      IF v_ownerAlreadyInNamespace > 0 THEN
        SELECT JSON_OBJECT('invitation', 'OWNER_ALREADY_IN_NAMESPACE')
        INTO validationErrors;
      ELSE
        ...
        IF v_duplicateNameCount > 0 THEN
          SELECT JSON_OBJECT('name', 'DUPLICATE_USER_NAME')
          INTO validationErrors;
        END IF;
      END IF;
    END IF;
