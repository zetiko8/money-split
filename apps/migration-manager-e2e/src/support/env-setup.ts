...
for (const envPath of envPaths) {
  if (fs.existsSync(envPath)) {
    ...
    console.log(`[migration-manager-e2e] Loading environment from: ${envPath}`);
...
if (!loaded) {
  // eslint-disable-next-line no-console
  console.warn('[migration-manager-e2e] No environment file found, using system environment variables');
}

// Log loaded configuration (without sensitive values)
// eslint-disable-next-line no-console
console.log('[migration-manager-e2e] Environment configuration:');
// eslint-disable-next-line no-console
