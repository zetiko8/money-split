...
for (const envPath of envPaths) {
  if (fs.existsSync(envPath)) {
    ...
    console.log(`[data-provider] Loading environment from: ${envPath}`);
...
if (!loaded) {
  // eslint-disable-next-line no-console
  console.warn('[data-provider] No environment file found, using system environment variables');
}

// Log loaded configuration (without sensitive values)
// eslint-disable-next-line no-console
console.log('[data-provider] Environment configuration:');
// eslint-disable-next-line no-console
console.log(`  NODE_ENV: ${process.env.NODE_ENV || 'NOT SET'}`);
// eslint-disable-next-line no-console
console.log(`  PORT: ${process.env.PORT || '3333 (default)'}`);
// eslint-disable-next-line no-console
